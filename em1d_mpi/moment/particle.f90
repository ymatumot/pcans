...
contains
  ...
  subroutine particle__solv(up,uf,                  &
                            c,q,r,delt,             &
                            np,nsp,np2,nxgs,nxge,bcp)

    integer, intent(in)    :: np, nxgs, nxge, nsp, bcp
    integer, intent(in)    :: np2(nxgs:nxge+bcp,nsp)
    real(8), intent(in)    :: uf(6,nxgs-1:nxge+1)
    real(8), intent(in)    :: c, q(nsp), r(nsp), delt
    real(8), intent(inout) :: up(4,np,nxgs:nxge+bcp,nsp)
    integer :: i, ii, isp, ih
